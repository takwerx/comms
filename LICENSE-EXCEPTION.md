# Licensing notes for Comms

Comms is licensed under the **[GNU Affero General Public License v3.0 or
...
>
> As a special exception, the copyright holders of Comms give you permission to
> combine, link with, and distribute Comms, or a work based on Comms, together
...
> resulting work that are Comms, or a work based on Comms.
>
...
**If you only install and use Comms, none of this touches you.** Running the
...
- `app/src/main/java/com/atakmap/android/comms/plugin/PluginNativeLoader.java`
...
- `app/src/main/java/com/atakmap/android/comms/plugin/Comms.java`
- `app/src/main/java/com/atakmap/android/comms/plugin/CommsPreferenceFragment.java`

**Third party:**

- `gradlew`, `gradlew.bat` and `gradle/wrapper/` — Gradle Inc., Apache License 2.0
- the catalog's data sources are named in the plugin itself, on the About screen,
  and in `catalog.json`'s `sources` list. They are public records published by
  Cal OES, the USDA Forest Service and the USGS; see
  [CONTRIBUTING.md](CONTRIBUTING.md) for what may be added to them.
